...
class Repository extends \PKP\submission\Repository
{
    ...
    protected function mapDashboardViews($types, Context $context, User $user, bool $canAccessUnassignedSubmission): Collection
    {
        ...
        return parent::mapDashboardViews($types, $context, $user, $canAccessUnassignedSubmission)
            ->filter(function ($item) use ($viewsToDisplay) {
                return $item && in_array($item->getData()['id'], $viewsToDisplay);
            });
